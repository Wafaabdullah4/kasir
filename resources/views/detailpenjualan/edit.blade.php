    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6">
                    <h1 class="text-2xl font-bold mb-4">Edit Detail Penjualan</h1>
                    <form action="{{ route('detailpenjualan.update', $detailPenjualan->id) }}" method="POST">
                        ...
                        <div class="mb-4">
                            ...
                            <input type="number" id="penjualanid" name="penjualanid"
                                value="{{ $detailPenjualan->penjualanid }}" class="mt-1 p-2 border rounded-md w-full">
                        </div>
                        <div class="mb-4">
                            ...
                            <input type="number" id="produkid" name="produkid"
                                value="{{ $detailPenjualan->produkid }}" class="mt-1 p-2 border rounded-md w-full">
                        </div>
                        <div class="mb-4">
                            ...
                            <input type="number" id="jumlah_produk" name="jumlah_produk"
                                value="{{ $detailPenjualan->jumlah_produk }}" class="mt-1 p-2 border rounded-md w-full">
                        </div>
                        <div class="mb-4">
                            <label for="subtotal" class="block text-sm font-medium text-gray-700">Subtotal:</label>
                            <input type="number" id="subtotal" name="subtotal" step="0.01"
                                value="{{ $detailPenjualan->subtotal }}" class="mt-1 p-2 border rounded-md w-full">
                        </div>
                        <div class="mt-4">
                            <button type="submit"
